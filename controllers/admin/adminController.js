...
exports.profile = async (req, res) => {
    ...
    Superadmin.findOne({ where: { id: req.session.user.id } }).then(function (BatchData) {
        ...
        res.render('profile', { title: 'Super Admin', admindetials: BatchData, userdetials: req.session.user });
    })
        .catch(err => {
            res.render('profile', { title: 'Super Admin', admindetials: BatchData, errorMessage: err });
        });
...
exports.superadminUpdate = async (req, res) => {
    ...
    Superadmin.update(batchDet, {
        ...
        Superadmin.findOne({ where: { id: req.session.user.id } }).then(function (BatchData) {
            console.log(BatchData);
            res.render('profile', { title: 'Super Admin', admindetials: BatchData, userdetials: req.session.user, successMessage: 'Updated Successfully.' });
        })
    })
        .catch(err => {
            res.render('profile', { title: 'Super Admin Edit', userdetials: req.session.user, errorMessage: err });
...
exports.changePassword = async (req, res) => {
    ...
    var superadmin = await Superadmin.findOne({ where: { id: req.session.user.id } });
